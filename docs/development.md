...
| `public/icons/`      | Toolbar / store **PNG** icons (`tajdin-radio-50.png`, `tajdin-radio-100.png`) — Chromium requires raster in `manifest.json`; refresh from `public/logo/*.svg` when updating art. Copied to `dist/icons/` by Vite |
| `public/logo/`       | Tajdîn marks: **SVG** (black/white — used in extension UI via `tajdinMarkSvgUrl()`), **PNG/JPG** (README, docs, exports), **PDF** (print). Black = light backgrounds; white = dark. All copied to `dist/logo/` |
