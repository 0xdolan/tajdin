...
- [ ] **Popup** opens from the toolbar icon and shows the basic Tajdîn UI (dark shell, title, main tabs including **About**, bottom **Player** bar: station line above controls, one row (no horizontal scroll): compact `h-7`/`h-8` controls, artwork default **Heroicons radio** outline when no favicon/cover, prev/play/next, random, plain **favourite heart** (no list-row frame), add-to-playlist, mute icon matches **volume 0 or muted** (else speaker-wave), volume slider `flex-1` into remaining width).
