...
- [ ] **Popup** opens from the toolbar icon and shows the basic Tajdîn UI (dark shell, title, main tabs including **About**, bottom **Player** bar: station line above controls, one horizontal row (`flex-nowrap`, horizontal scroll if needed): artwork, prev/play/next, random, **favourite heart**, add-to-playlist, mute + volume slider).
