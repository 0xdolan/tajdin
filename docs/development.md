...
| `src/settings/`      | Full-tab options UI: sidebar branding (`ExtensionBranding`, `titleTag="h1"`) + nav + `GeneralSettingsSection`, `CustomStationsTable` (+ `EditCustomStationModal`), `AboutSection`, reused `PlaylistsPage` / `GroupsPage`; listens to `chrome.storage.local` changes for `tajdin.*` (and legacy `zeng.*` during migration) to stay in sync with the popup |
