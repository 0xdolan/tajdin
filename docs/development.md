...
| `src/popup/`         | `App.tsx`: `PopupHeader` → `TabNav` → scrollable `main` (`TabPanel`) → **`PlayerDock`** (~88px, transport) → **`AttributionFooter`** (Developed with… / Source). `SurfaceContext` for light/dark. `components/`: `TabNav`, `Player`, `PlaylistsPage`, `AddStationModal`, `StationSearchBar`, `StationLanguageFilter`, `StationCard`, `StationList`; `playerPlayback.ts`; `playerBridge.ts`; `stationLibraryApi.ts`; `store/` + sync |
