...
| `src/popup/`         | `components/`: `Player`, `StationSearchBar`, `StationLanguageFilter` (browse `language` API param), `StationCard`, `StationList`; `hooks/useSearch.ts`; `playerBridge.ts`; `stationLibraryApi.ts`; `store/` + sync |
